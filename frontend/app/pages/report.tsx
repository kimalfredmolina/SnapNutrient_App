    ...
    <SafeAreaView style={{
      ...
      <ScrollView className="flex-1 px-4 py-6">
        <Text className="text-base mb-6 leading-6" style={{ color: colors.text, opacity: 0.8 }}>
          Help us improve SnapNutrients by reporting any issues you encounter. Your feedback is valuable to us!
        </Text>
          ...
          {reportTypes.map((type) => (
            ...
            >
              <View
                ...
                style={{
                  borderColor: selectedType === type.id ? colors.primary : colors.text + "40",
                  backgroundColor: selectedType === type.id ? colors.primary : "transparent",
                }}
              >
                {selectedType === type.id && <View className="w-3 h-3 rounded-full bg-white" />}
              </View>
              <Ionicons
        ...
        <View className="mb-6">
          ...
          <Text className="text-sm mb-3" style={{ color: colors.text, opacity: 0.7 }}>
        ...
        {/* Additional Info */}
        <View className="mb-8 p-4 rounded-lg" style={{ backgroundColor: colors.surface }}>
          <Text className="text-base font-medium mb-2" style={{ color: colors.text }}>
            Additional Information
          </Text>
          <Text className="text-sm leading-5" style={{ color: colors.text, opacity: 0.7 }}>
            • Device information and app version will be automatically included{"\n"}• Screenshots can help us
        ...
        <TouchableOpacity
          ...
          style={{
            backgroundColor: selectedType && description.trim() ? colors.primary : colors.text + "40",
          }}
          disabled={!selectedType || !description.trim()}
        >
          <Text className="text-center text-lg font-semibold text-white">Submit Report</Text>
        </TouchableOpacity>

        {/* Contact Alternative */}
        <View className="items-center py-4">
          <Text className="text-sm text-center mb-2" style={{ color: colors.text, opacity: 0.7 }}>
